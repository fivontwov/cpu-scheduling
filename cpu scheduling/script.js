...
function drawGanttChart() {
  ...
  if (selectedAlg === "FCFS") {
    ...
    for (let i = 0; i < processes.length; i++) {
      ...
      const startTime =
        currentTime >= process.arrivalTime ? currentTime : process.arrivalTime;
  ...
  if (selectedAlg === "SJF_N") {
    ...
    while (countdown > 0) {
      ...
      if (currentProcesses.length === 0) {
        currentTime = cloneProcesses[0].arrivalTime;
        continue;
      }
  ...
  if (selectedAlg === "SJF_P") {
    ...
    do {
      ...
      processes[index].waitingTime =
        currentTime - processes[index].arrivalTime - processes[index].burstTime;
      ...
      if (checked[index].checked === false) {
        processes[index].responseTime =
          currentTime - 1 - processes[index].arrivalTime;
    ...
    totalWaitingTime = processes.reduce(
      (total, process) => total + process.waitingTime,
      0
    );
    // Calculate the total turn around time
    totalTurnAroundTime = processes.reduce(
      (total, process) => total + process.turnAroundTime,
      0
    );
    // Calculate the total response time
    totalResponseTime = processes.reduce(
      (total, process) => total + process.responseTime,
      0
    );
  }
  ...
  if (selectedAlg === "RR") {
    ...
    do {
      ...
      let endTime =
        currentTime + quantumTime > currentTime + process.burstTime
      ...
      cloneProcesses.push({
        name: process.name,
        arrivalTime: process.arrivalTime,
        burstTime: process.burstTime - (endTime - currentTime),
        priority: process.priority,
      });
  ...
  averageWaitingTime.innerHTML = (totalWaitingTime / processes.length).toFixed(
    2
  );

  const averageTurnAroundTime = document.getElementById(
    "avg-turnaround-time-value"
  );
  averageTurnAroundTime.innerHTML = (
    totalTurnAroundTime / processes.length
  ).toFixed(2);

  const averageResponseTime = document.getElementById(
